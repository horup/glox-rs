...
impl Camera {
    ...
    pub fn screen_ray(&self, screen_pos: Vec2) -> Ray {
        ...
        let ndc_x = (2.0 * screen_pos.x) / self.viewport_size.x - 1.0;
        let ndc_y = 1.0 - (2.0 * screen_pos.y) / self.viewport_size.y;
    ...
    pub fn world_to_screen(&self, world_pos: Vec3) -> Vec2 {
        ...
        let p = p.xy();
        Vec2::new(
            (p.x + 1.0) / 2.0 * self.viewport_size.x,
            (1.0 - p.y) / 2.0 * self.viewport_size.y,
        )
    }
}
